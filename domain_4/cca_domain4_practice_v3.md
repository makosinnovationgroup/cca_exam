...
33 questions covering Prompt Engineering & Structured Output (20% of the real exam). Calibrated **harder than the real exam** — distractors include realistic-but-wrong patterns (asking for JSON in the prompt, retrying without limits, applying Batches API to real-time scenarios) and verbatim-fact traps (incorrect retention windows, fabricated beta header strings). The final 3 questions (Q31-Q33) were added during the v3 coverage audit to close hard gaps against the official exam guide.

## How to use

- [ ] **Round 1** — cold attempt all 33 without checking answers
...
|------|-----------|-------|
| 4.1 | [[#^d4-q-1\|Q1]]-[[#^d4-q-4\|Q4]], [[#^d4-q-31\|Q31]] | Explicit criteria, clear and direct prompts, false-positive category handling |
...
| 4.5 | [[#^d4-q-21\|Q21]]-[[#^d4-q-26\|Q26]], [[#^d4-q-32\|Q32]] | Message Batches API: cost, latency, capabilities, appropriateness criteria |
| 4.6 | [[#^d4-q-27\|Q27]]-[[#^d4-q-30\|Q30]], [[#^d4-q-33\|Q33]] | Multi-instance independent review, per-file + integration pass architecture |
...
### Gap-closure questions (audit v3 additions)

#### Question 31

A code-review agent has 5 finding categories. Over 3 weeks of production use, 2 categories have produced findings at a 38% false-positive rate while the other 3 categories are reliable. Developers have begun ignoring findings from **all** categories — trust has eroded across the board. The team is rewriting the prompts for the 2 problem categories but the work will take 2 more weeks. What's the most effective interim response? ^d4-q-31

A. Lower the global confidence threshold for all categories until the prompts are rewritten.
B. Temporarily disable the 2 high-false-positive categories entirely while their prompts are being rewritten; keep the 3 accurate categories running. Restoring trust in the categories that work is more valuable than partial coverage that contaminates reception of the accurate findings.
C. Apply a system-prompt-level "be more conservative" instruction across all 5 categories until the rewrite completes.
D. Continue running all 5 categories but add a confidence label to each finding so developers can filter mentally.

> [!success]- Reveal answer
> **Correct: B**
>
> The documented pattern for developer-trust restoration is **surgical, not blanket**: isolate the high-false-positive categories and disable them while improving their prompts. The accurate categories continue producing valuable signal during the rewrite window.
>
> **Why others are wrong:**
> - A: Lowering thresholds globally degrades the accurate categories to dampen the bad ones — penalizing what works.
> - C: A blanket "be conservative" instruction does the same: it degrades the categories that were already calibrated correctly. Distractor specifically called out in the exam guide.
> - D: Continuing to ship false positives further erodes trust; filter-mentally puts the burden on the developer the agent was supposed to help.

#### Question 32

A team operates four Claude-powered workflows and wants to reduce API costs: **(1)** nightly test-suite generation that runs at 2am and posts results to Slack by morning standup; **(2)** pre-merge CI code review that gates every PR with feedback shown to developers within 90 seconds; **(3)** a weekly compliance audit that produces a Monday-9am report from data collected over the prior week; **(4)** a customer-facing chat assistant. Which workflows are appropriate for the Message Batches API? ^d4-q-32

A. All four — the Batches API saves 50% across the board and all workflows can tolerate the wait.
B. Only the pre-merge CI checks — they have the highest cost per PR.
C. Only the customer-facing chat — it has the highest call volume so savings compound.
D. The nightly test generation and the weekly compliance audit — both are non-blocking and latency-tolerant. The pre-merge CI checks (90-second human-blocking expectation) and the customer-facing chat (real-time UX) cannot tolerate the up-to-24-hour Batches SLA.

> [!success]- Reveal answer
> **Correct: D**
>
> The decision rule is **"will a human or downstream system be waiting on this result within the next hour?"** If yes → synchronous. If no → Batches.
> - Nightly test gen: completes overnight, posted to Slack by morning — non-blocking → **Batches**.
> - Weekly compliance audit: data collected over prior week, report Monday 9am — non-blocking → **Batches**.
> - Pre-merge CI: blocks a PR with 90-second expected feedback → **synchronous**.
> - Customer-facing chat: real-time conversational UX → **synchronous**.
>
> **Why others are wrong:**
> - A: Pre-merge CI and customer chat cannot tolerate the 24-hour Batches SLA — saving 50% on cost is worthless if the workflow breaks.
> - B & C: Backwards — picks the workflows that are *worst* fit for Batches.

#### Question 33

A pull request modifies 14 files across the payments module. The team's single-instance, single-pass review approach catches local issues (syntax, style, single-file logic) but has missed two cross-file API contract bugs in recent sprints. Logs show the model's reasoning quality on files 8–14 is materially worse than on files 1–7. What architectural change best addresses this? ^d4-q-33

A. Two-stage multi-pass review: **(1) per-file local passes** — one Claude instance per file with isolated context, focused on local issues; **(2) cross-file integration pass** — one instance reviewing inter-file data flow and API contracts, with the structured summaries from the per-file passes as input. The per-file split avoids attention dilution; the integration pass recovers cross-file analysis on a reduced surface.
B. Submit all 14 files in a single prompt with a longer, more detailed review checklist requiring the agent to verify each item explicitly.
C. Run 3 separate Claude instances each reviewing all 14 files with slightly different prompts, then merge the findings.
D. Add a CI hook that automatically rejects any PR exceeding 10 files and forces the developer to split it.

> [!success]- Reveal answer
> **Correct: A**
>
> The exam guide tests this exact pattern in sample Q12 with a 14-file PR scenario. The architecture is **per-file local + cross-file integration**, in two stages. Per-file passes get full attention on local correctness; the integration pass operates on summaries (reduced surface) so cross-file reasoning has room to work.
>
> **Why others are wrong:**
> - B: Single-pass review of 14 files in one prompt is exactly the failure mode the question describes — longer checklist doesn't fix attention dilution.
> - C: Running the same single-pass anti-pattern three times in parallel doesn't address dilution; it triples the cost of the same broken approach.
> - D: A CI gate punishes the developer for the agent's architectural limitation; doesn't fix the agent.
...
| [[#^d4-q-31\|31]] | B | [[#^d4-q-32\|32]] | D | [[#^d4-q-33\|33]] | A | | | | | | |

## Stats summary

- **Total questions:** 33 (30 base + 3 gap-closure additions from audit v3)
- **Answer distribution:** A=9 (27%) · B=8 (24%) · C=8 (24%) · D=8 (24%)
- **Hardest questions** (subtle distinctions, two-look-right options): [[#^d4-q-3\|Q3]] (vagueness comparison), [[#^d4-q-4\|Q4]] (prompt vs schema), [[#^d4-q-14\|Q14]] (required + nullable vs optional), [[#^d4-q-16\|Q16]] (when retry helps), [[#^d4-q-23\|Q23]] (Batches misapplication), [[#^d4-q-30\|Q30]] (Batches + multi-instance review composition), [[#^d4-q-32\|Q32]] (multi-workflow Batches decision)
- **Trap questions:** [[#^d4-q-23\|Q23]] (looks like a cost-saving win, tests SLA trap), [[#^d4-q-27\|Q27]] (looks reasonable to self-review but fails on independence), [[#^d4-q-28\|Q28]] (anchoring rationale, not just "no"), [[#^d4-q-31\|Q31]] (global-fix distractors look defensible)
...
- **Gap-closure questions (audit v3):** [[#^d4-q-31\|Q31]] (Task 4.1 — false-positive category disabling), [[#^d4-q-32\|Q32]] (Task 4.5 — Batches-appropriate workflows), [[#^d4-q-33\|Q33]] (Task 4.6 — per-file + integration pass architecture)
