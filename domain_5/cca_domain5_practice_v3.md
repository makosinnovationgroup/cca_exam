...
31 questions covering Context Management & Reliability (15% of the real exam). Calibrated **harder than the real exam** — distractors include realistic-but-wrong patterns (sentiment-based escalation, record-level confidence, prose summarization of evidence) and fabricated-but-plausible SDK fields and beta header strings. The final question (Q31) was added during the v3 coverage audit to close a hard gap against the official exam guide.

## How to use

- [ ] **Round 1** — cold attempt all 31 without checking answers
...
| 5.3 | [[#^d5-q-11\|Q11]]-[[#^d5-q-15\|Q15]], [[#^d5-q-31\|Q31]] | Error propagation across agent boundaries, synthesis coverage annotations |
...
A research synthesis pipeline produces reports across multiple subagents. On a recent topic ("regulatory environment for autonomous vehicles in Southeast Asia"), 3 of 7 subagent searches returned empty or low-coverage results due to source unavailability — current Southeast Asian regulatory text wasn't accessible. The synthesis renders all findings in uniform prose paragraphs, treating well-supported and undersupported sections identically. Stakeholders have begun citing the report as confident evidence for areas that actually had thin source backing. What structural change best addresses this? ^d5-q-31

A. Add explicit **coverage annotations** to the synthesis output: structure the report to distinguish well-supported findings (3+ sources) from gap-flagged areas where sources were unavailable or contradictory. Preserve in the structured output which subagent searches returned empty so readers — and downstream agents — can see what *wasn't* covered, not just what was.
B. Increase the synthesis agent's temperature so language conveys more uncertainty in undersupported areas.
C. Have the synthesis agent re-run with a stricter "confident findings only" instruction that drops any topic where coverage was thin.
D. Have a human reviewer go through the synthesis and insert disclaimers where coverage was weak.

> [!success]- Reveal answer
> **Correct: A**
>
> The documented pattern is **structural surfacing of uncertainty**, not prose hedging. Coverage annotations distinguish well-supported from gap-flagged areas in the output schema; preserving which subagent searches returned empty makes the absence of evidence auditable, not invisible. This is the same root principle as cite-where-possible (Distractor Pattern 4) and silent reconciliation (Distractor Pattern 5): prose loses; schema preserves.
>
> **Why others are wrong:**
> - B: Higher temperature produces noisier text, not better coverage signaling. Prose hedges are probabilistic and lossy.
> - C: Dropping thin-coverage areas hides the gap entirely — the stakeholder no longer knows the topic was undersupported, so they can't ask follow-ups or commission more research.
> - D: Human-inserted disclaimers don't scale, don't propagate to downstream agents, and rely on the reviewer noticing what's missing in prose — exactly the failure mode the question describes.
...
- **Total questions:** 31 (30 base + 1 gap-closure addition from audit v3)
- **Answer distribution:** A=8 (26%) · B=8 (26%) · C=7 (23%) · D=8 (26%)
- **Hardest questions** (subtle distinctions, two-look-right options): [[#^d5-q-2\|Q2]] (memory tool protocol details), [[#^d5-q-6\|Q6]] (architecture vs model "memory"), [[#^d5-q-14\|Q14]] (partial-vs-complete distinction rationale), [[#^d5-q-21\|Q21]] (compaction vs context editing trade-offs), [[#^d5-q-23\|Q23]] (field-level vs record-level confidence), [[#^d5-q-30\|Q30]] (structural vs prose synthesis), [[#^d5-q-31\|Q31]] (coverage annotations vs prose hedging)
- **Trap questions:** [[#^d5-q-7\|Q7]] and [[#^d5-q-10\|Q10]] (sentiment-based escalation anti-pattern), [[#^d5-q-22\|Q22]] (random vs stratified sampling), [[#^d5-q-23\|Q23]] (record-level confidence is the wrong unit), [[#^d5-q-31\|Q31]] (prose-hedging distractors look reasonable)
...
- **Gap-closure question (audit v3):** [[#^d5-q-31\|Q31]] (Task 5.3/5.6 — coverage annotations for unavailable-source areas)
